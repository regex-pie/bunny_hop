...
function animateHop(token, startX, startY, endX, endY) {
    ...
    return new Promise((resolve) => {
        ...
        function tick() {
            ...
            // 旋转
            if(startX < endX){  //向右走
                console.log(`tokenRot=${tokenRot} , MAX_ROTATION=${MAX_ROTATION} , p=${p}`);
...
async function playHopPath(token, waypoints) {
    ...
    try {
        ...
        console.log("✅ 逐格动画完成，终点:", waypoints[waypoints.length - 1]);
    } catch (err) {
        console.error("❌ playHopPath 出错:", err);
        throw err
    }
}
...
function handleTokenMoveStep(tokenDocument, movementData, operation) {
    console.log("触发钩子handleTokenMoveStep");
    ...
    console.log(`⏭️ Token: ${tokenId} 正在动画中，阻止新拖拽`);
        return false;
    }

    const token = canvas.tokens.get(tokenDocument.id);
    if (!token) {
        console.warn("❌ Token 不在场景");
        return false;
    }
    ...
    console.log(`📌 完整路径（${fullPath.length} 个点）`);


	// 发送emit
	console.log(`emit启动：tokenId=${tokenDocument.id} , fullPath=${fullPath} , fullPath.len=${fullPath.length}`);
...
Hooks.once('ready', () => {
	...
	game.socket.on(SOCKET_EVENT, (data) => {
		...
		console.log(`收到Socket 事件 ${SOCKET_EVENT} ,token=${tokenId}, fullPath=${fullPath}`);
	    // 锁定该Token，防止并发
	    _movingTokens.add(tokenId);

	    //移动token
	    playHopPath(canvas.tokens.get(tokenId), fullPath)
        .then(() => {
            console.log("🔄 远程动画完成");
        })
        .catch(err => {
            console.error("❌ 远程动画播放失败:", err);
        })
        .finally(() => {
            _movingTokens.delete(tokenId);
        });

		console.log(`收到Socket 事件 ${SOCKET_EVENT} ,token=${tokenId}, 处理结束.`);
	});
    console.log(`✅ Socket 事件 ${SOCKET_EVENT} 监听已注册`);
